...
float Compute_OBJECT4DV1_Radius(OBJECT4DV1_PTR obj)
{
    ...
    // loop throught and compute radius
                        ...
                        VECTOR4D_PTR rot)       // initial rotations
{
    ...
    {
        Write_Error("\n Couldn't open PLG file %s.", filename);
        return(0);
    } // end if
                    ...
                    int cull_flags)     // clipping planes to consider
{
    ...
    // frustum by using the sent camera information and object
                                          ...
                                          CAM4DV1_PTR cam)
{
    ...
    for (int poly = 0; poly < rend_list->num_polys; poly++)
    {
        ...
        for (int vertex = 0; vertex < 3; vertex++)
        {
            ...
            // not that we are NOT dividing by the homogeneous w coordinate since
            // we are not using a matrix operation for this version of the function 
	                                 ...
	                                 int lighting_on)
{
    ...
    for (int poly = 0; poly < obj->num_polys; poly++)
    {
		...
		//test if use high-16bit color to override low-16bit color
		unsigned int base_color;
		
		if (lighting_on)
		{
			base_color = curr_poly->color;
			curr_poly->color = curr_poly->color >> 16;
		}
                                ...
                                UCHAR *video_buffer, int lpitch)
{
    ...
    for (int poly=0; poly < rend_list->num_polys; poly++)
    {
        ...
        Draw_Clip_Line16(rend_list->poly_ptrs[poly]->tvlist[0].x, 
            ...
            rend_list->poly_ptrs[poly]->color,
            video_buffer, lpitch);
		if (rend_list->poly_ptrs[poly]->tvlist[0].x > 600)
		{
			char buffer[255];
			sprintf(buffer, "x[%d]", 1);
			Draw_Text_GDI(buffer, 0, 40, RGB(255, 0, 0), lpddsback);
		}
                  ...
                  float viewport_height)
{
    ...
    // first set up params that are no brainers
    ...
    // set the view-plane dimensions up, they will be 2 x (2/ar)
    cam->viewplane_width  = 2.0;
	cam->viewplane_height = 2.0/cam->aspect_ratio;

    // now we know fov and we know the view-plane dimensions plug into formula and
    // solve for view distance parameters
    float tan_fov_div2 =1/tan(DEG_TO_RAD(fov/2));
